...
/** Verifica se headers da planilha contêm todos os esperados. Retorna lista de ausentes. */
...
export function validateMedicaoRow(row: MedicaoRow, index: number): string[] {
  ...
  if (
    row.TEMPERATURA_01 !== null &&
    (!t1.success || t1.data === undefined || !Number.isFinite(t1.data))
  )
    issues.push(`[${index}] TEMPERATURA_01 não-finito: ${row.TEMPERATURA_01}`);

  const t2 = temperaturaSchema.safeParse(row.TEMPERATURA_02);
  if (
    row.TEMPERATURA_02 !== null &&
    (!t2.success || t2.data === undefined || !Number.isFinite(t2.data))
  )
...
const backlogPrioridadeSchema = z
  ...
  .default("")
  .transform((v) => v.toLowerCase())
  .pipe(z.enum(["", "alta", "média", "media", "baixa", "normal", "urgente", "critico"]).catch(""));
